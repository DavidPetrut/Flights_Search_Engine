...
@Component({
  selector: 'app-second-search-engine',
  templateUrl: './second-search-engine.component.html',
  styleUrls: ['./second-search-engine.component.css']
})
export class SecondSearchEngineComponent implements OnInit {
...
searchFlights() {
  if (this.whereFrom && this.whereTo && this.selectDate) {
    ...
    console.log(`[SearchEngine] Departure Matches:`, departureMatches);
    ...
    console.log(`Initiating search with criteria: From ${this.whereFrom} to ${this.whereTo} on ${startDate} to ${endDate}`);

    console.log(`[SearchEngine] Data to Pass to Results:`, dataToPass);
...
searchSingleFlight() {
  ...
  if (this.whereFrom && this.whereTo && this.selectDate) {
    ...
    console.log("Searching single flights with criteria:", {
    ...
    const singleTickets = singleMatches.map(flight => ({
      ...
      // Add any other properties you need from the flight
    }));
    ...
    const dataToPass: FlightData = {
      singleTickets: singleTickets, // This comes from mapping your single flight matches
      numberOfAdults: this.numberOfAdults,
      numberOfKids: this.numberOfKids,
      searchCriteria: {
        from: this.whereFrom,
        to: this.whereTo,
        startDate: formatDate(this.selectDate, 'yyyy-MM-dd', 'en-US'), // Note: Adjusted for single date
        endDate: formatDate(this.selectDate, 'yyyy-MM-dd', 'en-US'), // Same as startDate for single flights
        passengers: this.numberOfPassengers
      }
    };
...
//   whereTo = "";
//   currentDate = new Date();
//   selectDate: Date | Date[] | null = null;
//   locations: any[] = [];
//   flights: any[] = [];

//   constructor(private route: ActivatedRoute, private http: HttpClient, private router: Router, private flightDataService: FlightDataService, private searchCriteriaService: SearchCriteriaService) {}
...
//   searchFlights() {
//     if (!this.selectDate || !Array.isArray(this.selectDate)) {
//         console.error('Invalid date selection');
//         return;
//     }

//     const formattedDates = this.parseDates(this.selectDate);
//     const combinedTickets = this.generateCombinedTickets(formattedDates.startDate, formattedDates.endDate);
...
//     if (dataToPass.searchCriteria) {

//       console.log(`[SecondSearchEngine] Data to Pass to Results:`, dataToPass);

//         this.flightDataService.changeFlightData(dataToPass);
//         this.router.navigate(['/results'], {
//             queryParams: {
//                 from: dataToPass.searchCriteria.from,
//                 to: dataToPass.searchCriteria.to,
//                 startDate: formattedDates.startDate,
//                 endDate: formattedDates.endDate,
//                 passengers: dataToPass.searchCriteria.passengers.toString()
//             }
//         });
//     }
//   }

//   generateCombinedTickets(startDate: string, endDate: string): CombinedTicket[] {
//     // Assuming startDate and endDate are in 'yyyy-MM-dd' format and need to be converted to 'MM/dd/yyyy'
//     const formatStartDate = formatDate(new Date(startDate), 'MM/dd/yyyy', 'en-US');
//     const formatEndDate = formatDate(new Date(endDate), 'MM/dd/yyyy', 'en-US');
  
//     const departureFlights = this.flights.filter(flight =>
//       flight.fullNameDeparture.includes(this.whereFrom) && flight.flightDay === formatStartDate
//     );
  
//     const returnFlights = this.flights.filter(flight =>
//       flight.fullNameArrival.includes(this.whereTo) && flight.flightDay === formatEndDate
//     );

//     console.log(`[SecondSearchEngine] Departure Flights:`, departureFlights);
//     console.log(`[SecondSearchEngine] Return Flights:`, returnFlights);

  
//     // Explicitly typed as CombinedTicket[]
//     let combinedTickets: CombinedTicket[] = departureFlights.flatMap(departureFlight => {
...
//           totalNewTicketPrice: departureFlight.newTicketPrice + returnFlight.newTicketPrice,
//         };
//       }).filter(ticket => ticket !== null); // Ensure all entries are valid
//     });

//     console.log(`[SecondSearchEngine] Combined Tickets:`, combinedTickets);
...
//       return { startDate: '', endDate: '' };
//     }
//     // Format dates to match your JSON data format (MM/dd/yyyy)
//     const startDate = formatDate(dateRange[0], 'MM/dd/yyyy', 'en-US');
//     const endDate = formatDate(dateRange[1], 'MM/dd/yyyy', 'en-US');
//     console.log("hi");
